...
def custom_activation(x):
    return tf.cast(tf.math.greater(x, tf.constant([0.0])), tf.float32)


# Arquitetura da rede
model = Sequential()

# A função degrau não é diferenciável!
model.add(
    Dense(
        2,
        input_dim=2,
...
model.add(
    Dense(
        1, kernel_initializer="zeros", activation=custom_activation, name="output_layer"
    )
)
